## a9202507@gmail.com
## adding parallel load function 
## 2023_10_25: Adding Rigol function generator 
...
class chromaEload(gpibChromaMachine):
    ...
    # for eload parallel using.
    def total_load_on(self):
        self.inst.write("CHAN 3;LOAD ON")
        
    # for eload parallel using.
    def total_load_off(self):
        self.inst.write("CHAN 3;LOAD OFF")
        
    # for eload parallel using.
    def setTotalCurrent(self,current):
        self.current = current
        self.inst.write('CURR:STAT:L1 '+str(current))
...
class rigol_equipment(visa_equipment):
    def __init__(self, visa_resource_name):
        visa_equipment.__init__(self, visa_resource_name)

class rigol_function_generator(rigol_equipment):
    def __init__(self, visa_resource_name):
        rigol_equipment.__init__(self, visa_resource_name)

    def set_wavform_sheap(self,channel,sheap='SQU'):
        print(f':SOUR{str(channel)} FUNC {sheap}')
        self.inst.write(f':SOUR{str(channel)}:FUNC {sheap}')
